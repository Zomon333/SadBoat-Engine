...
#---------------------------------------------------
CC = g++
FLAGS = -std=c++20 -fpermissive -pthread
...
prod:
	mkdir ./output &
	g++ $(FLAGS) $(DEFINES) $(PRODUCTION_RELEASE) $(SOURCE_DIR)/*.cpp $(INCLUDE_DIRS) -L$(LIBRARY_DIR) $(LIBS) -o $(OUT_DIR)/$(NAME)$(VERSION)"" 2>&1 | tee $(LOG_NAME)

#Compiles for debug
debug:
	mkdir ./output &
	g++ $(FLAGS) $(DEFINES) $(PRODUCTION_RELEASE) $(SOURCE_DIR)/*.cpp $(INCLUDE_DIRS) -L$(LIBRARY_DIR) $(LIBS) -o $(OUT_DIR)/$(NAME)$(VERSION)"-dbg" 2>&1 | tee $(LOG_NAME)
...
test:
	mkdir ./output &
	g++ $(FLAGS) $(DEFINES) $(TEST_RELEASE) $(SOURCE_DIR)/*.cpp $(INCLUDE_DIRS) $(LIBRARY_DIR) $(LIBS) -o $(OUT_DIR)/$(NAME)$(VERSION)"-T" 2>&1 | tee $(LOG_NAME)
